...
def createTempCSV(testName, numCLI, nodeTypes, nodeSplit, nodeType, nodeNum):
    ...
    if (len(videoStarts) == 0 and newDF['DASHVideoPlaybackPointerValues'].dropna().tolist()[-1] != 0.0):
        videoStarts = [newDF['DASHVideoPlaybackPointerTS'].dropna().tolist()[-1]]
...
def recalculateQoENode(testName, numCLI, nodeTypes, nodeSplit, nodeType, nodeNum, segLen):
    ...
    for i in range(numVids):
        tmp = csv2j.toJson(os.path.join(file_dir,'../tempCSV/'+testName+nodeType+str(nodeNum)+'vid'+str(i)+'.csv'),os.path.join(file_dir,'../tempJSON/'+testName+nodeType+str(nodeNum)+'vid'+str(i)+'.json'),testName+nodeType+str(nodeNum)+'vid'+str(i),segLen)
        if tmp:
