...
#     VL=ds['PlaybackPointer'].dropna().iloc[-1]
    ...
    # print(0, VL, seglen, tmp)
    tmp['start']=np.arange(0,VL,seglen)
    tmp = tmp.rename(columns={'QualityLevel':'bitrate', 'Resolution':'resolution'})
...
'''
def toJson(input_file,output_file,name,segLen):
    streamID=0
    ...
    ds.columns=['tsBufferLength','BufferLength','tsReceivedBytes','ReceivedBytes','tsQualityLevel','QualityLevel','tsResolution','Resolution','tsPlaybackPointer','PlaybackPointer','tsPlaybackStatus','PlaybackStatus']
    # print('\n' + ds.to_string())
    ...
    ds['tsResolution'] = ds['tsResolution'] - initialTime
...
if __name__ == "__main__":
    print('CSV to JSON translation with segment length of ', end='')
    input_file=sys.argv[1]
    output_file=sys.argv[2]
    name = sys.argv[3]
    segLen = sys.argv[4]
    print(int(segLen), end='')
    print(' for node ', end='')
    print(name, end=' processing... ')
    toJson(input_file,output_file,name,int(segLen))
    print('Done!')
